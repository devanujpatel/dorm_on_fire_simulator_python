            ...
            for tile in self.fire_list:
                ...
                if x - 1 >= 0 and y - 1 >= 0 and self.all_tiles_dict[x - 1][y - 1] is not None and \
                    ...
                    self.all_tiles_dict[x - 1][y - 1].set_on_fire(self.canvas,
                                                                  ...
                                                                  self.width, self.height, "tomato")

                    # top up same column
                if y - 1 >= 0 and self.all_tiles_dict[x][y - 1] is not None and self.all_tiles_dict[x][
                    y - 1].flammable == "Yes":
                    # print("past2")
                    self.all_tiles_dict[x][y - 1].set_on_fire(self.canvas,
                                                              self.all_tiles_dict[x][y - 1].x * self.width,
                                                              self.all_tiles_dict[x][y - 1].y * self.height,
                                                              self.width,
                                                              self.height, "gold")
                ...
                if x - 1 >= 0 and self.all_tiles_dict[x - 1][y] is not None and self.all_tiles_dict[x - 1][
                    y].flammable == "Yes":
                    # rint("past4")
                    self.all_tiles_dict[x - 1][y].set_on_fire(self.canvas,
                                                              self.all_tiles_dict[x - 1][y].x * self.width,
                                                              self.all_tiles_dict[x - 1][y].y * self.height,
                                                              self.width,
