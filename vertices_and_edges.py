class VerticesAndEdges:
    ...
    def work(self):
        for tile in self.all_tiles_list:
            ...
            # top left cell
            if x - 1 >= 0 and y - 1 >= 0 and self.all_tiles_dict[x - 1][y - 1] is not None:
                ...
                # top up same column
            if y - 1 >= 0 and self.all_tiles_dict[x][y - 1] is not None:
                ...
                # top right
            if x + 1 <= 99 and y - 1 >= 0 and self.all_tiles_dict[x - 1][y - 1] is not None:
                self.tile_to_neighbouring_tiles[tile].append(self.all_tiles_dict[x + 1][y - 1])
                self.tile_to_neighbouring_tiles_costs[tile][self.all_tiles_dict[x + 1][y - 1]] = \
                self.all_tiles_dict[x + 1][y - 1].cost_imposed_on_weight

                # same level left
            if x - 1 >= 0 and self.all_tiles_dict[x - 1][y] is not None:
                self.tile_to_neighbouring_tiles[tile].append(self.all_tiles_dict[x - 1][y])
                self.tile_to_neighbouring_tiles_costs[tile][self.all_tiles_dict[x - 1][y]] = self.all_tiles_dict[x - 1][
                    y].cost_imposed_on_weight

                # same level right
            if x + 1 <= 99 and self.all_tiles_dict[x + 1][y] is not None:
                self.tile_to_neighbouring_tiles[tile].append(self.all_tiles_dict[x + 1][y])
                self.tile_to_neighbouring_tiles_costs[tile][self.all_tiles_dict[x + 1][y]] = self.all_tiles_dict[x + 1][
                    y].cost_imposed_on_weight

                # bottom left
            if x - 1 >= 0 and y + 1 <= 99 and self.all_tiles_dict[x - 1][y + 1] is not None:
                self.tile_to_neighbouring_tiles[tile].append(self.all_tiles_dict[x - 1][y + 1])
                self.tile_to_neighbouring_tiles_costs[tile][self.all_tiles_dict[x - 1][y + 1]] = \
                self.all_tiles_dict[x - 1][y + 1].cost_imposed_on_weight

            # same column bottom down
            if y + 1 <= 99 and self.all_tiles_dict[x][y + 1] is not None:
                self.tile_to_neighbouring_tiles[tile].append(self.all_tiles_dict[x][y + 1])
                self.tile_to_neighbouring_tiles_costs[tile][self.all_tiles_dict[x][y + 1]] = self.all_tiles_dict[x][
                    y + 1].cost_imposed_on_weight

            # bottom right
            if x + 1 <= 99 and y + 1 <= 99 and self.all_tiles_dict[x + 1][y + 1] is not None:
                self.tile_to_neighbouring_tiles[tile].append(self.all_tiles_dict[x + 1][y + 1])
                self.tile_to_neighbouring_tiles_costs[tile][self.all_tiles_dict[x + 1][y + 1]] = \
                self.all_tiles_dict[x + 1][y + 1].cost_imposed_on_weight
